...
// Store network logs at module level so afterEach can access them
let networkLogs = [];

test.afterEach(async ({}, testInfo) => {
  // Always save network logs after test (pass or fail)
  if (networkLogs.length > 0) {
    const filename = `test-results/${testInfo.title.replace(/\s+/g, "-")}-network.json`;
    saveNetworkLogs(networkLogs, filename);
    console.log(
      "Network Summary:",
      JSON.stringify(getNetworkSummary(networkLogs), null, 2),
    );
  }
});
...
test("Excel data based automation", async ({ page }) => {
  ...
  networkLogs = setupNetworkCapture(page);
  ...
  for (let i = 46; i < data.length; i++) {
    ...
    try {
      ...
      await page
        .locator("span")
        .filter({ hasText: "Pick a Limit" })
        .first()
        .click();
      await page.getByText(row.QutSelContLimit).click();
      await page
        .locator("span")
        .filter({ hasText: "Pick a Deductible" })
        .first()
        .click();
      ...
      await page
        .locator(
          "div[class='col-sm-6 ng-star-inserted'] td[class='ng-star-inserted']",
        )
        .click();
      await page.waitForTimeout(3000);
      ...
      const balanceText = await page
        .locator("span[class='ng-star-inserted'] b")
        .innerText();
      ...
      await page
        .locator("nb-accordion-item-header")
        .filter({ hasText: "Client Information" })
        .click();
      ...
      await page
        .locator(
          "//ng2-smart-table//tbody/tr[1]/td[1]//input[@type='checkbox']",
        )
        .check();
      ...
      await page.getByRole("button", { name: "Yes" }).click();
      await page.goto(
        "https://www.landydev.com/#/pages/depositBatchTab/DepositBatch",
      );
      await page.getByRole("link").filter({ hasText: /^$/ }).nth(3).click();
      await page.getByRole("button", { name: "Yes" }).click();
      await page.locator("//ng2-smart-table//tbody/tr[1]/td[7]//a").click();
      ...
      await page.locator("#search").first().click();
      await page
        .locator("//tr[@class='ng2-smart-row selected ng-star-inserted']")
        .click();
      await page.locator("//nb-tabset//ul/li[1]/a").click();
      await page.getByRole("link", { name: "Payment", exact: true }).click();
      ...
      // Network logs are saved in afterEach hook
