...
// -------- Excel Read --------
const workbook = xlsx.readFile('./tests/Data/APEXPAllState.xlsx');
...
test('Excel data based automation', async ({ page }) => {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    try {
      ...
      await page.getByPlaceholder('Applicant Name').click();
      await page.getByPlaceholder('Applicant Name').type(row.FirmName);
      await page.getByRole('textbox', { name: 'Sizing example input' }).nth(2).click();
      await page.getByRole('textbox', { name: 'Sizing example input' }).nth(2).type(row.Location);
      await page.waitForTimeout(3000);
      ...
      await page.waitForTimeout(3000);
      await page.locator('input[name="effDate2"]').fill(new Date().toISOString().split('T')[0]);
      ...
      await page.getByPlaceholder('No of Disciplinary Actions').click();
      await page.locator('//*[@id="noOfDisActions"]').fill(row.AppDetlFullTimeProfessionals);
      await page.waitForTimeout(3000);
      await page.locator('#ftCpa').click();
      await page.getByRole('textbox', { name: '$' }).press('ArrowLeft');
      await page.getByRole('textbox', { name: '$' }).press('ArrowLeft');
      await page.getByRole('textbox', { name: '$' }).press('ArrowLeft');
      await page.locator('#ftCpa').type(row.TotalIncurredLosses);
      await page.getByRole('button', { name: 'save & Close' }).click();
      await page.getByRole('button', { name: ' Next' }).click();
      await page.waitForTimeout(3000);
       // ********************Upload File************************
      const fileInput = page.locator('input[type="file"]');
      await fileInput.setInputFiles('C:/Users/vpmgo/Downloads/ENDO_828197.pdf');
      // **********************************************************
      await page.locator("//tr[3]//td[2]//div[1]").click();
      await page.waitForTimeout(3000);
